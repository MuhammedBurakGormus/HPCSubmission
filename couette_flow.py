...
######
def couette_flow(width:int,height:int,w:float,u_wall_x:float,time_steps:int,plot_time_steps:int,output_loc:str=None): 
    """
    This function simulates Couette flow, and returns the final probability density function. 
    ...
    Besides, it plots&saves 2D velocity profiles in the x direction at t=time_steps.
    Args: 
        width: int 
    ...
    ## get the final pdf & check mass conservation in the end ## 
    pdf_initial = f_time[0,:,:,:]
    pdf_last = f_time[time_steps,:,:,:]
    #check_mass_conservation(pdf_initial,pdf_last,"Couette Flow")
    ...
    velocity_field = calculate_velocity_field(pdf_last)
    ...
    visualize_velocity_field_x(velocity_field,title=title,add_wall=True,pos=["Bottom","Top"],output_name=output_name,output_loc=output_loc) 

    return f_time[-1,:,:,:] # return final pdf 
