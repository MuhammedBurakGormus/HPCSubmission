...
######
def parallelized_sliding_lid(width:int,height:int,N:int,M:int,v:float,u_wall_x:float,time_steps:int,plot_time_steps:int,output_loc:str=None):
    """
    ...
    # Make the inputs compatible with the variables below
    ...
    for time_step in range(0,time_steps):
        ...
        if ((time_step+1) % plot_time_steps == 0) and ( time_step !=0 ):
            ...
            if rank == 0:
                ...
                output_name_0 = f"rank_0_parallel_sliding_lid_u={u_wall[0]}_v={v}_width={width}_height={height}_time={time_step+1}_Re={Re}_N={N}_M={M}"
                streamplot_velocity(velocity_field_0,output_name=output_name_0,output_loc=output_loc,kinematic_vis=v,u_wall=u_wall[0],Re=Re,time=time_step+1)
                ...
                output_name = f"parallel_sliding_lid_u={u_wall[0]}_v={v}_width={width}_height={height}_time={time_step+1}_Re={Re}_N={N}_M={M}"
...
# To run the code from the terminal, following command to be used:  (NxM processor)
# mpirun -n NxM python3 parallelized_sliding_lid.py 
